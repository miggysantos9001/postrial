...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        {!! Form::model($booking,['method'=>'PATCH','action'=>['BookingController@update',$booking->id]]) !!}
    	<div class="row">
    		<div class="col-md-12">
                                ...
                                <table class="table" style="font-size: 10px;">
                                    ...
                                    <tbody>
                                        ...
                                        <?php 
                                            ...
                                            $consume = \App\Booking_detail::where('booking_id',$booking->id)
                                                ->where('product_id',$data->id)
                                                ->get();

                                            $donated = \App\Donation_detail::where('date',$booking->date)
                                                ->where('client_id',$booking->client_id)
                                                ->where('product_id',$data->id)
                                                ->get();    
                                            ...
                                            if($data->isHead == 1){
                                                ...
                                                $d = $donated->sum('weight');
                                            }
                                        ?>
                                        <tr>
                                            <td>{{ $data->name }}</td>
                                            <td style="font-size:14px;font-weight: bolder;margin-top: -5px;" class="text-center">{{ number_format($total,0) }}</td>
                                            <td style="font-size:14px;font-weight: bolder;margin-top: -5px;" class="text-center">{{ number_format($total - ($prodGet + $d),0) }}</td>
                                            @foreach($customers as $row)
                                            <?php 
